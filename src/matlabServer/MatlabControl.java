...
public class MatlabControl {
	...
	/*************** CONSTRUCTORS ****************/
	/***
	 * usually, the default constructor with no arguments is fine. Sometimes, a
	 * callback function is useful. A callback function is allows the user to
	...
	/********** TEST FUNCTIONS ***********************/
	/***
	 * call these functions from within Matlab itself. These are examples of the
	 * general execution order: 1. instantiate java object from matlab 2. spawn
	 * a new Java thread 3. call matlab functions from new java thread
	 * 
	 * EXAMPLE (from matlab prompt):
	 * 
	 * >> mc=MatlabControl; >> mc.testEval('x = 5') x = 5 >
	 * mc.testFeval('help',{'sqrt'}) SQRT Square root. SQRT(X) is the square
	 * root of the elements of X. Complex results are produced if X is not
	 * positive.
	 * 
	 * See also SQRTM.
	 * 
	 ...
	 * 
	 * >> mc.testBlockingFeval('sqrt',{x}) 2.2361
	 ****/
	...
	protected class MatlabEvalCommand implements Runnable {
		...
		protected Object useMatlabCommandCallback(String command, Object[] args) {
			...
			try {
				return matlab.mtFevalConsoleOutput(
						new String("matlabControlcb"), newArgs, 0);
			} catch (Exception e) {
				// System.out.println(e.toString());
				return null;
			}
