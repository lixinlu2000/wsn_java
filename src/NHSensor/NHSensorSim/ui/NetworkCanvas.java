...
import NHSensor.NHSensorSim.tools.ImageUtils;
import NHSensor.NHSensorSim.util.Convertor;

public class NetworkCanvas extends JPanel implements NetworkView {
	...
	private String nodeImageFileName = "./images/MICAz_gross.jpg";
	private double nodeImageScale = 0.03;

	/*
	 * private int xScale = 1; private int yScale = 1;
	 */
	private boolean drawRadioRange = true;
	protected boolean isShowDescription = false;
	...
	public NetworkCanvas(NetworkModel networkModel) {
		super();
		int width,height;
		...
		try {
			BufferedImage resizedImage = ImageUtils.getResizePicture(ImageIO.read(new File(nodeImageFileName)),
					nodeImageScale);
			nodeImage = ImageUtils.createImageByMaskColorEx(resizedImage, Color.white);
		} catch (IOException e) {
	...
	public void drawSelectedNodes(Graphics2D g2d) {
		Node node;
		Vector nodes = this.getNetworkModel().getSelectedNodes();
		for (int i = 0; i < nodes.size(); i++) {
			node = (Node) nodes.elementAt(i);
			this.drawSelectedNode(g2d, node);
		}
	}
	...
	public void drawNode(Graphics2D g2d, Node node) {
		this.drawNodeImage(g2d, node, true);
	}
	
	public void drawSelectedNode(Graphics2D g2d, Node node) {
		int w = nodeImage.getWidth();
		int h = nodeImage.getHeight();
		
		Point2D.Double pointReal = this.getCoordinator().point(Convertor.positionToPoint2DDouble(node.getPos()));
		
		Rectangle2D.Double rectangle2D = new Rectangle2D.Double(
				pointReal.getX() - w/2, pointReal.getY() - h/2, w,
				h);
		Color oldColor = g2d.getColor();
		g2d.setColor(selectedNodeColor);
	...
	public void drawNodeImage(Graphics2D g2d, Node node, boolean fill) {
		this.drawImageForNode(g2d, Convertor.positionToPoint2DDouble(node.getPos()));
		if (this.isShowNodeID) {
			this.drawString(g2d, Integer.toString(node.getId()), node.getPos()
					.getAddedPosition(new Position(0.1, 0.1)), this
					.getNodeColor());
	...
	public int getNodeWidth() {
		return  nodeImage.getWidth();
	}
	
	public int getNodeHeight() {
		return nodeImage.getHeight();
	}
	
	public void drawImageForNode(Graphics2D g2d, Point2D.Double point) {
		int w = nodeImage.getWidth();
		int h = nodeImage.getHeight();
		
		Point2D.Double pointReal = this.getCoordinator().point(point);
		
//		Rectangle2D.Double rectangle2D = new Rectangle2D.Double(
//				point.getX() - w/2, point.getY() - h/2, w,
//				h);
//		Rectangle2D.Double rectangle2DReal = this.getCoordinator().rectangle(rectangle2D);

		//g2d.draw(rectangle2DReal);

		g2d.drawImage(this.nodeImage, (int)(pointReal.getX()-w/2), (int)(pointReal.getY()-h/2),
				w, h, null);
	}
