 ...
 */
public class EDCCircleTraverseRectIteratorCDAFailureAware extends
		TraverseRectByCircleIterator implements TraverseShapeOptimizeIterator,
		TraverseRectRepairIterator {
	private final double MINOR_DOUBLE = 0.00001;
	private EDCCircleTraverseRectIteratorCDAUseEndBound ectrDD;
	private EDCCircleTraverseRectIteratorCDARepair repairIterator;

	public EDCCircleTraverseRectIteratorCDAFailureAware(Algorithm alg,
			int direction, Rect rect) {
		super(alg, direction, rect);
		ectrDD = new EDCCircleTraverseRectIteratorCDAUseEndBound(alg,
				direction, rect);
		repairIterator = new EDCCircleTraverseRectIteratorCDARepair(alg,
				direction, rect);
	}
		...
		if (curNa == null && curShape == null) {
			...
			if (!curShape.contains(curNa.getNode().getPos())) {
				...
				if (nextShape == null)
					...
					curNa.collectNeighborLocations(this.shapeCalNextNa(curNa, curShape, false));
	...
	protected Shape shapeCalNextNa(NeighborAttachment curNa, Shape curShape,
			boolean isOptimize) {
		if (curShape instanceof Rect) {
			return this.caculateNextNaAndShape(curNa, (Rect) curShape, isOptimize).getShapeCalNextNa();
		} else if (curShape instanceof CircleCircleInRect) {
			return this.caculateNextNaAndShape(curNa, (CircleCircleInRect) curShape,
					isOptimize).getShapeCalNextNa();
		} else if (curShape instanceof CircleBoundInRect) {
			return this.caculateNextNaAndShape(curNa, (CircleBoundInRect) curShape,
					isOptimize).getShapeCalNextNa();
		} else if (curShape instanceof BoundCircleInRect) {
			return this.caculateNextNaAndShape(curNa, (BoundCircleInRect) curShape,
					isOptimize).getShapeCalNextNa();
		} else if (curShape instanceof BoundBoundInRect) {
			return this.caculateNextNaAndShape(curNa, (BoundBoundInRect) curShape,
					isOptimize).getShapeCalNextNa();
		} else
			return null;
	}
	...
	protected NeighborAttachment nextNeighborAttachment(
			NeighborAttachment curNa, Vector nasInCurShape,
			Vector nasInNextShape, boolean isOptimize) {
		double radioRange = this.alg.getParam().getRADIO_RANGE();
		if (nasInNextShape == null || nasInNextShape.size() == 0)
			return null;

		Collections.sort(nasInNextShape, new RelativeDistanceComparator(curNa
				.getPos()));
		NeighborAttachment nextNa = null, candidateNa, prevNa;
		boolean isNextNa;
		for (int i = 0; i < nasInNextShape.size(); i++) {
			candidateNa = (NeighborAttachment) nasInNextShape.elementAt(i);
			isNextNa = true;
			for (int j = 0; j < i; j++) {
				prevNa = (NeighborAttachment) nasInNextShape.elementAt(j);
			...
			if (isOptimize) {
				for (int j = 0; j < nasInCurShape.size(); j++) {
					prevNa = (NeighborAttachment) nasInCurShape.elementAt(j);
					if (prevNa.getPos().distance(candidateNa.getPos()) > radioRange) {
						isNextNa = false;
						break;
					}
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		BoundCircleInRect shape1 = new BoundCircleInRect(
				curShape.getBeginBound(this.getDirection()), curNaRadioCircle,
				this.getRect(), this.direction);
		Shape shapeCalNextNa = isOptimize?shape1:maxNextBoundCircleInRect;
		...
		if (nas.isEmpty()) {
			return new NextNaAndShape(null, nextShape, shapeCalNextNa);
		} else {
			NeighborAttachment nextNa = this.nextNeighborAttachment(curNa,
					nasInCurShape, nas, isOptimize);
			//
			if (nextNa == null) {
				return new NextNaAndShape(null, nextShape, shapeCalNextNa);
			}
			...
			if (this.unIntersectedCircleInRect(newEndCircle, this.direction,
					this.rect)) {
				NextNaAndShape nextNaAndShape =  this.ectrDD.caculateNextNaAndShape(curNa, curShape,
						isOptimize);
				nextNaAndShape.setShapeCalNextNa(shapeCalNextNa);
				return nextNaAndShape;
			} else {
			...
			return new NextNaAndShape(nextNa, nextShape, shapeCalNextNa);
		}
	}
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		CircleCircleInRect maxNextCircleCircleInRect = new CircleCircleInRect(
				...
				this.direction);
		CircleCircleInRect shape1 = new CircleCircleInRect(
				curShape.getEndCircle(), curNaRadioCircle,
				this.getRect(), this.direction);
		CircleCircleInRect shapeCalNextNa = isOptimize?shape1:maxNextCircleCircleInRect;
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		BoundCircleInRect maxNextBoundCircleInRect = new BoundCircleInRect(
				...
				this.direction);
		CircleCircleInRect shape1  = new CircleCircleInRect(
				curShape.getBeginCircle(), curNaRadioCircle, this.getRect(),
				this.direction);
		Shape shapeCalNextNa = isOptimize?shape1:maxNextBoundCircleInRect;
		Shape nextShape = maxNextBoundCircleInRect;
		if (this.isEnd(maxNextBoundCircleInRect)) {
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		CircleCircleInRect maxCircleCircleInRect = new CircleCircleInRect(
				...
				this.direction);
		BoundCircleInRect shape1 = new BoundCircleInRect(
				curShape.getBeginBound(), curNaRadioCircle, this.getRect(),
				this.direction);
		Shape shapeCalNextNa = isOptimize?shape1:maxCircleCircleInRect;	
	...
	public NeighborAttachment getOptimizeNextNeighborAttachment(
			NeighborAttachment curNa, Shape curShape) {
		curNa.collectNeighborLocations(this.shapeCalNextNa(curNa, curShape, true));
	...
	public NeighborAttachment getRepairedNeighborAttachment(
			...
			NeighborAttachment curNa, Shape curShape) {
		return repairIterator.getNextNeighborAttachment(prevNa, prevShape);
	}
	...
	public int getRepairedNeighborAttachmentNum(NeighborAttachment prevNa,
		...
		return repairIterator.repairNodeNum(prevNa, prevShape);
	}

	public Shape getRepairedShape(NeighborAttachment prevNa, Shape prevShape,
			NeighborAttachment curNa, Shape curShape) {
		return repairIterator.getNextShape(prevNa, prevShape);
	}
