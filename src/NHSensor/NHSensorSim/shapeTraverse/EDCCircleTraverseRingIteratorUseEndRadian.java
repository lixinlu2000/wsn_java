...
public class EDCCircleTraverseRingIteratorUseEndRadian extends
		TraverseRingIterator {
	RingSector firstRingSector;
	static final double MINI_DOUBLE_VALUE = 0.0001;

	public EDCCircleTraverseRingIteratorUseEndRadian(Algorithm alg,
	...
	protected double caculateMaxSita() {
		...
		return EDCCircleTraverseRingIteratorUseEndRadian
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		if (curShape instanceof RingSector) {
			...
			circle = new Circle(curNa.getPos(), this.getAlg().getParam()
					.getRADIO_RANGE());
			endRadian = ShapeUtil.circleIntersectRingMinEndRadian(circle,
					this.ring);
			double sita = Radian.relativeTo(endRadian, curRingSector
					.getEndRadian());
			maxNextShape = new RingSector(this.ring, curRingSector
					.getEndRadian(), sita);
			...
			nextNa = this.nextNodeByRadian(curNa, neighborNodes);
			if (nextNa == null) {
				...
				sita = Radian.relativeTo(endRadian, curRingSector
						.getEndRadian());
				nextShape = new RingSector(this.ring, curRingSector
						.getEndRadian(), sita);
				return new NextNaAndShape(nextNa, nextShape);
			}
		} else if (curShape instanceof AngleCircleInRing) {
			...
			endRadian = ShapeUtil.circleIntersectRingMinEndRadian(circle,
					this.ring);
			maxNextShape = new CircleAngleInRing(curAngleCircleInRing
			...
			if (nextNa == null) {
				...
				endRadian = this.ring.getCircleCentre()
						.bearing(nextNa.getPos())
						+ MINI_DOUBLE_VALUE;
				nextShape = new CircleAngleInRing(curAngleCircleInRing
						.getCircle(), endRadian, this.getRing());
				return new NextNaAndShape(nextNa, nextShape);
			}
		} else if (curShape instanceof CircleCircleInRing) {
			...
			maxNextShape = new CircleAngleInRing(curCircleCircleInRing
					.getCircleEnd(), endRadian, this.getRing());
			if (this.isEnd(maxNextShape)) {
				maxNextShape = this.truncateShape(maxNextShape);
			}
			Vector neighborNodes = curNa.getNeighbors(this.getAlg().getParam()
			...
			if (nextNa == null) {
				...
				endRadian = this.ring.getCircleCentre()
						.bearing(nextNa.getPos())
						+ MINI_DOUBLE_VALUE;
				nextShape = new CircleAngleInRing(curCircleCircleInRing
						.getCircleEnd(), endRadian, this.getRing());
				return new NextNaAndShape(nextNa, nextShape);
			}
		} else if (curShape instanceof CircleAngleInRing) {
			...
			endRadian = ShapeUtil.circleIntersectRingMinEndRadian(circle,
					this.ring);
			double sita = Radian.relativeTo(endRadian, curCircleAngleInRing
					.getAngle());
			maxNextShape = new RingSector(this.getRing(), curCircleAngleInRing
					.getAngle(), sita);
			if (this.isEnd(maxNextShape)) {
				maxNextShape = this.truncateShape(maxNextShape);
			}
			...
			if (nextNa == null) {
				...
				endRadian = this.ring.getCircleCentre()
						.bearing(nextNa.getPos())
						+ MINI_DOUBLE_VALUE;
				sita = Radian.relativeTo(endRadian, curCircleAngleInRing
						.getAngle());
				nextShape = new RingSector(this.getRing(), curCircleAngleInRing
						.getAngle(), sita);
				return new NextNaAndShape(nextNa, nextShape);
