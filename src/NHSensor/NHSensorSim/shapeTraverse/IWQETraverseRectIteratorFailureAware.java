 ...
 */
public class IWQETraverseRectIteratorFailureAware extends TraverseRectIterator {
	static Logger logger = Logger
			.getLogger(IWQETraverseRectIteratorFailureAware.class);

	public IWQETraverseRectIteratorFailureAware(Algorithm alg, int direction,
			Rect rect) {
		...
		if (curNa == null) {
			...
			if (curRect != null) {
				if (curRect.contains(curNa.getNode().getPos())) {
					...
					curNa.collectNeighborLocations(shape1);
					...
					if (next == curNa)
						...
						// IWQETraverseRectIteratorFailureAware
						// and IWQETraverseRectIterator
						if (!next.isAlive()) {
				...
				Rect nextRect = this.nextRect(curNa);
				double radioRange = this.alg.getParam().getRADIO_RANGE();
				Circle curNaRadioCircle = new Circle(curNa.getPos(), radioRange);
				BoundCircleInRect shape1 = new BoundCircleInRect(
						nextRect.getBeginBound(this.getDirection()), curNaRadioCircle,
						this.getRect(), this.direction);
				curNa.collectNeighborLocations(shape1);
