...
public class EDCRingSectorTraverseRingIterator extends TraverseRingIterator {
	RingSector firstRingSector;
	static final double MINI_DOUBLE_VALUE = 0.0001;

	public EDCRingSectorTraverseRingIterator(Algorithm alg,
	...
	protected double caculateMaxSita() {
		...
		return EDCRingSectorTraverseRingIterator.calculateRingSectorSita(
		...
		if (curShape instanceof RingSector) {
			...
			if (nextNa == null) {
				...
				circle = new Circle(curNa.getPos(), nextNa.getPos().distance(
						curNa.getPos())
						+ EDCRingSectorTraverseRingIterator.MINI_DOUBLE_VALUE);
