...
/*
 * grid traverse a rect region using GBA
 * ex means this class implements TraverseShapeOptimizeIterator interface
 */
public class EDCGridTraverseRectIteratorEx extends TraverseRectIterator
		implements TraverseShapeOptimizeIterator {

	public EDCGridTraverseRectIteratorEx(Algorithm alg, int direction, Rect rect) {
	...
	public NeighborAttachment getOptimizeNextNeighborAttachment(
		...
		neigbhorsInCurRect = curNa.getNeighbors(curRect);
		...
		NeighborAttachment na0, na;
		NeighborAttachment nextAttachment = curNa;
		for (int i = neigbhorsInNextRect.size() - 1; i >= 0; i--) {
			na = (NeighborAttachment) neigbhorsInNextRect.elementAt(i);
			...
			if (j == i) {
				int k = 0;
				int size = neigbhorsInCurRect.size();
				for (k = 0; k < size; k++) {
	...
	public Shape getOptimizedNextShape(NeighborAttachment curNa, Shape curShape) {
		...
		nextRect = this.minRectContainNode(nextRect, direction, nextAttachment);
