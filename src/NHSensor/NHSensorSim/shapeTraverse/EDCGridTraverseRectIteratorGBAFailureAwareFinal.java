 ...
 */
public class EDCGridTraverseRectIteratorGBAFailureAwareFinal extends
	...
	EDCGridTraverseRectIteratorGBAOptimize optimizeIterator;

	public EDCGridTraverseRectIteratorGBAFailureAwareFinal(Algorithm alg,
			int direction, Rect rect) {
		super(alg, direction, rect);
		repairIterator = new EDCGridTraverseRectIteratorGBARepair(alg,
				direction, rect);
		optimizeIterator = new EDCGridTraverseRectIteratorGBAOptimize(alg,
				direction, rect);
	}
		...
		if (curNa == null && curRect == null) {
			...
			if (!curRect.contains(curNa.getNode().getPos())) {
				...
				if (nextRect == null)
					...
					curNa.collectNeighborLocations(this.shapeCalNextNa(curNa, curRect));
	...
	public Shape shapeCalNextNa(NeighborAttachment curNa, Rect curRect) {		
		...
		BoundCircleInRect shape1 = new BoundCircleInRect(
				curRect.getEndBound(this.getDirection()), curNaRadioCircle,
	...
	public NeighborAttachment getOptimizeNextNeighborAttachment(
			NeighborAttachment curNa, Shape curShape) {
		return this.optimizeIterator.getOptimizeNextNeighborAttachment(curNa,
				curShape);
	}

	public Shape getOptimizedNextShape(NeighborAttachment curNa, Shape curShape) {
		return this.optimizeIterator.getOptimizedNextShape(curNa, curShape);
	}
}
