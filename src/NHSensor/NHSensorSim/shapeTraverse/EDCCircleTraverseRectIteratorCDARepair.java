 ...
 * iterator to repair
 */
public class EDCCircleTraverseRectIteratorCDARepair extends
		TraverseRectByCircleIterator {
	private final double MINOR_DOUBLE = 0.00001;
	private EDCCircleTraverseRectIteratorCDAUseEndBoundRepair ectrDD;

	public EDCCircleTraverseRectIteratorCDARepair(Algorithm alg, int direction,
			Rect rect) {
		super(alg, direction, rect);
		ectrDD = new EDCCircleTraverseRectIteratorCDAUseEndBoundRepair(alg,
				direction, rect);
	}
	...
	public int repairNodeNum(NeighborAttachment curNa, Shape curShape) {
		Shape nextshape = this.getNextShape(curNa, curShape);
		Vector neighbors = curNa.getNeighbors(nextshape);
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
			BoundBoundInRect curShape) {
		return this.caculateNextNaAndShape(curNa, curShape.toRect());
	}
	...
	protected NeighborAttachment nextNeighborAttachment(
		...
		Collections.sort(nasInNextRect, new RelativeDistanceComparator(curNa
				.getPos()));
