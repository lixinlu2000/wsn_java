 ...
 */
public class EDCCircleTraverseRectIteratorCAAUseEndBoundLinkAwareLifeTime extends
	...
	double rectSizeScale = 10;
	
	public EDCCircleTraverseRectIteratorCAAUseEndBoundLinkAwareLifeTime(Algorithm alg,
	...
	public Rect getInitialDestRect(NeighborAttachment root) {
		...
		case Direction.LEFT:
			...
			width = Math.sqrt(radioRange * radioRange - height * height)/rectSizeScale;
		...
		case Direction.DOWN:
			...
			height = Math.sqrt(radioRange * radioRange - width * width)/rectSizeScale;
	...
	protected double viaNodeCostNoOptimize(NeighborAttachment curNa,
		...
		NeighborAttachment nextNa = (NeighborAttachment)sortedNasInMaxNextShape.elementAt(nextNaIndex);
		double energies[] = new double[boundNaIndex+1];
		double curNaEnergy, nextNaEnergy;
		double queryMessageSize = this.getAlg().getParam().getQUERY_MESSAGE_SIZE();
		double answerMessageSize = this.alg.getParam().getANSWER_SIZE();
		double times, broadcastQueryMessageTimes=0, collectedNodeNum, totalCost;

		double minCount = Double.MAX_VALUE;
		double count;

		nextNaEnergy = nextNa.getReceiveEnergy(curNa, answerMessageSize);
		nextNaEnergy += nextNa.getReceiveEnergy(curNa, queryMessageSize);
		curNaEnergy = curNa.getSendEnergy(nextNa, answerMessageSize);
		
		for (int i = 0; i <= boundNaIndex; i++) {
			na = (NeighborAttachment) sortedNasInMaxNextShape.elementAt(i);
			energies[i] = na.getReceiveEnergy(curNa, queryMessageSize);
			energies[i] += na.getSendEnergy(nextNa, answerMessageSize);

			nextNaEnergy += nextNa.getReceiveEnergy(na, answerMessageSize);
			times = curNa.getSendTimes(na);
			broadcastQueryMessageTimes = Math.max(times,
					broadcastQueryMessageTimes);
			
			count = Math.floor(na.getEnergy()/energies[i]);
		...
		curNaEnergy += broadcastQueryMessageTimes*curNa.getOneSendEnergy(null, queryMessageSize);
		if (nextNaIndex > boundNaIndex) {
			collectedNodeNum = boundNaIndex + 2;
		} else
			collectedNodeNum = boundNaIndex + 1;

		count = Math.floor(curNa.getEnergy()/curNaEnergy);
		if(count<minCount) minCount = count;
		count = Math.floor(nextNa.getEnergy()/nextNaEnergy);
		if(count<minCount) minCount = count;
		return -minCount*collectedNodeNum;
	}
	...
	protected double viaNodeTimes(NeighborAttachment curNa,
		...
		double totalCost;
		if (isOptimize)
			totalCost = this.viaNodeCostOptimizeForTimes(curNa,
					sortedNasInMaxNextShape, nextNaIndex, boundNaIndex);
		else
			totalCost = this.viaNodeCostNoOptimizeForTimes(curNa,
					sortedNasInMaxNextShape, nextNaIndex, boundNaIndex);
		return totalCost / this.alg.getParam().getANSWER_SIZE();
	}
	
	
	protected double viaNodeCostOptimizeForTimes(NeighborAttachment curNa,
			Vector sortedNasInMaxNextShape, int nextNaIndex, int boundNaIndex) {
		double totalCost = 0;
	...
	protected double viaNodeCostNoOptimizeForTimes(NeighborAttachment curNa,
			Vector sortedNasInMaxNextShape, int nextNaIndex, int boundNaIndex) {
		double totalCost = 0;
		double collectAnswerCost = 0;
		NeighborAttachment na;
	...
	protected double viaNodeCostOptimize(NeighborAttachment curNa,
		...
		curNaEnergy += broadcastQueryMessageTimes*curNa.getOneSendEnergy(null, queryMessageSize);
		if (nextNaIndex > boundNaIndex && nextNa.isHasSendAnswer()) {
			collectedNodeNum = boundNaIndex + 2;
		} else
			collectedNodeNum = boundNaIndex + 1;
		
		int curNaIndex = sortedNasInMaxNextShape.indexOf(curNa);
		if(curNaIndex<=boundNaIndex&&curNa.isHasSendAnswer())
			collectedNodeNum--;
		
		if(curNaIndex>boundNaIndex&&curNaIndex!=nextNaIndex&&!curNa.isHasSendAnswer())collectedNodeNum++;
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		if (nas.isEmpty()) {
			...
			cost = this.viaNodeCost(curNa, nas, nas.indexOf(nextNa), nas.indexOf(boundNa), isOptimize);
			//
			if (nextNa == null) {
				return new NextNaAndShape(null, nextShape);
			}
			...
			BoundBoundInRect newNextShape = new BoundBoundInRect(
					isOptimize ? curShape.getBeginBound(this.getDirection())
							: curShape.getEndBound(this.getDirection()),
					boundNa.getPos().getBound(direction, this.MINOR_DOUBLE),
					this.getRect(), this.direction);
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		if (nas.isEmpty()) {
			...
			CircleBoundInRect newNextShape = new CircleBoundInRect(curShape
					.getEndCircle(), boundNa.getPos().getBound(direction,
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		if (nas.isEmpty()) {
			...
			BoundBoundInRect newNextShape = new BoundBoundInRect(curShape
					.getEndBound(), boundNa.getPos().getBound(direction,
					this.MINOR_DOUBLE), this.getRect(), this.direction);
			nextShape = newNextShape;
			if (this.isEnd(newNextShape)) {
				nextShape = this.truncateShape(newNextShape);
