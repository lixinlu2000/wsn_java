...
public class EDCAdaptiveGridTraverseRingIteratorFailureAware extends
	...
	public EDCAdaptiveGridTraverseRingIteratorFailureAware(Algorithm alg,
	...
	protected double caculateMaxSita() {
		...
		return EDCAdaptiveGridTraverseRingIteratorFailureAware
	...
	protected RingSector caculateNextRingSector(NeighborAttachment curNa,
		...
		for (int i = candidateNodes.size() - 1; i >= 0; i--) {
			...
			// added code 2010-6-8
			// if(candidateNode.isDestroy())continue;
			//
			isNextNa = true;
			for (int j = 0; j < i; j++) {
				candidateNodePrev = (NeighborAttachment) candidateNodes
						.elementAt(j);
				// added code 2010-6-8
				// if(candidateNodePrev.isDestroy())continue;
				//
	...
	public NeighborAttachment getRepairedNeighborAttachment(
		...
		RingSector curRingSector = (RingSector) curShape;
		Vector candidateNodes = curNa.getNeighbors(curShape);
		RelativeRadianComparator comparator = new RelativeRadianComparator(
				curRingSector.getStartRadian());
		Collections.sort(candidateNodes, comparator);

		double radioRange = this.getAlg().getParam().getRADIO_RANGE();
		NeighborAttachment na, na0;
		curNa.destroy();
		int maxNaIndex = -1; // The max index that can communicate with the
								// nodes has index less that the max index
		for (int i = 0; i < candidateNodes.size(); i++) {
			na = (NeighborAttachment) candidateNodes.elementAt(i);

			int j;
			for (j = 0; j < i; j++) {
				na0 = (NeighborAttachment) candidateNodes.elementAt(j);
	...
	public Shape getRepairedShape(NeighborAttachment prevNa,
			NeighborAttachment curNa, Shape curShape) {
		RingSector curRingSector = (RingSector) (((RingSector) curShape)
				.clone());
		Vector candidateNodes = curNa.getNeighbors(curRingSector);
		RelativeRadianComparator comparator = new RelativeRadianComparator(
				curRingSector.getStartRadian());
		Collections.sort(candidateNodes, comparator);
		...
		if (nextNa == null) {
			...
			this.setCannotTraverse(false);
			curRingSector.setEndRadian(this.ring.getCircleCentre().bearing(
					nextNa.getNode().getPos())
					+ MIN_DOUBLE_VALUE);
			// check if reach the end of the ring
			double relativeSita = Radian.relativeTo(this.firstRingSector
					.getStartRadian(), curRingSector.getStartRadian());
			if (curRingSector.getSita() > relativeSita)
				curRingSector.setEndRadian(this.firstRingSector
						.getStartRadian());
			return curRingSector;
		}
	}
