...
public class EDCAdaptiveGridTraverseRingIteratorOld extends
		TraverseRingIterator {
	RingSector firstRingSector;
	final static double MIN_DOUBLE_VALUE = 0.0000001;

	public EDCAdaptiveGridTraverseRingIteratorOld(Algorithm alg,
	...
	protected double caculateMaxSita() {
		...
		return EDCAdaptiveGridTraverseRingIteratorOld.calculateRingSectorSita(
		...
		if (nextNa != null) {
			...
			for (int i = 0; i < candidateNodes.size(); i++) {
				...
				for (j = 0; j < i; j++) {
					...
					if (na.getNode().getPos().distance(na0.getNode().getPos()) > r) {
						break;
					}
				}
				if (j == i) {
			...
			if (nextNa == null) {
				...
				nextRingSector.setEndRadian(this.ring.getCircleCentre()
						.bearing(nextNa.getNode().getPos())
						+ MIN_DOUBLE_VALUE);
			}
