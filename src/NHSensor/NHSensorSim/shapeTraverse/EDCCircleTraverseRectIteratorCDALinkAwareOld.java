 ...
 */
public class EDCCircleTraverseRectIteratorCDALinkAwareOld extends
		TraverseRectByCircleIterator implements TraverseShapeOptimizeIterator {
	private final double MINOR_DOUBLE = 0.00001;
	private EDCCircleTraverseRectIteratorCDAUseEndBoundLinkAware ectrDD;

	public EDCCircleTraverseRectIteratorCDALinkAwareOld(Algorithm alg,
			int direction, Rect rect) {
		super(alg, direction, rect);
		ectrDD = new EDCCircleTraverseRectIteratorCDAUseEndBoundLinkAware(alg,
				direction, rect);
	}
	...
	protected NextNaAndShape caculateNextNaAndShape(NeighborAttachment curNa,
		...
		if (nas.isEmpty()) {
			...
			Collections.sort(nas, new RelativeDistanceHasDirectionComparator(
					curNa.getPos(), this.direction));
