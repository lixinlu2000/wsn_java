...
/*
 * The difference between IWQETraverseRingIteratorNew and IWQETraverseRingIterator
 * is IWQETraverseRingIteratorNew is much more simple.
 */
public class IWQETraverseRingIteratorNew extends TraverseRingIterator {
	RingSector firstRingSector;
	static final double MINI_DOUBLE_VALUE = 0.0001;
	double prevNaRelativeRadian = 0;

	public IWQETraverseRingIteratorNew(Algorithm alg, NeighborAttachment root,
			Ring ring) {
		super(alg, root, ring);
	...
	protected double caculateMaxSita() {
		...
		return IWQETraverseRingIteratorNew.calculateRingSectorSita(lowRadius,
...
//			if (this.ring.contains(curNa.getPos())) {
			if ( (curShape!=null&&curShape.contains(curNa.getPos())) ||
					(curShape==null&&this.ring.contains(curNa.getPos()))) {
...
//			if (this.ring.contains(curNa.getPos())) {
	...
	protected RingSector caculateNextRingSector(NeighborAttachment curNa) {
		...
		nextRingSector = new RingSector(this.getRing(), curNaBearing+MINI_DOUBLE_VALUE, this
	...
	public boolean isEnd(NeighborAttachment curNa, Shape curShape,
		...
		if (curShape != null) {
			if(this.isReachTheEndOfRing((RingSector) curShape)) 
				return true;

			if(curShape!=this.firstRingSector) {
				return this.firstRingSector.contains(curNa.getPos());
			}
			else return false;
		} else if (curNa != null) {
