...
import NHSensor.NHSensorSim.algorithm.HSAApproximateAlg;
...
public class HSAApproximateEnergyExperiment extends AlgorithmExperiment {
	AllParam param;
	...
	public HSAApproximateEnergyExperiment(AllParam param) {
		this.param = param;
		SensorSim sensorSim = SensorSim.createSensorSim(param);
		sensorSim.getSimulator().addHandleAndTraceEventListener();

		HSAApproximateAlg alg = (HSAApproximateAlg) sensorSim.generateAlgorithm(HSAApproximateAlg.NAME);
		...
		//UniformDataSet dataset = new UniformDataSet(this.getParam().getDatasetMax(),this.getParam().getDatasetID());
		UniformDataSet dataset = new UniformDataSet(946.37, 1077.9,this.getParam().getDatasetID());
		//ShaanxiLoessPlateauDataSet dataset = new ShaanxiLoessPlateauDataSet();
		alg.setDataset(dataset);
		alg.setDelta(this.getParam().getDelta());
		alg.initSectorInRectsBySita((int) this.param.getSubAreaNum());
