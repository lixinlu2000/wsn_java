package NHSensor.NHSensorSim.papers.EST.lifeTime;
...
public class ExperimentUtil {
	...
	String[] algNames = new String[] { "FullFlood", "SWinFlood", "EcstaFlood",
			"EST_GQP_RDC", "EST_GQP_IDC", "EST_MQP_RDC", "EST_MQP_IDC",
			"EST_MQP_IDC_ByArea" };
	boolean[] algRunOrNot = new boolean[] { true, true, true, true, true, true,
			true, false };
	...
	public ExperimentParamResultPairCollections energy(String variableName,
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if (algRunOrNot[3])
				algorithmExperiments[3] = new RestaFarEnergyExperiment(param);
			if (algRunOrNot[4])
				algorithmExperiments[4] = new RestaIDCFarEnergyExperiment(param);
			if (algRunOrNot[5])
				algorithmExperiments[5] = new RestaFarMulticastQueryEnergyExperiment(
						param);
			if (algRunOrNot[6])
				algorithmExperiments[6] = new RestaIDCFarMulticastQueryEnergyExperiment(
						param);
			if (algRunOrNot[7])
				algorithmExperiments[7] = new RestaIDCFarMulticastQueryPartitionByAreaEnergyExperiment(
						param);
