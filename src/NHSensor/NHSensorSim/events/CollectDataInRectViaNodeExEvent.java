 ...
 * has a endRect member variable
 */

public class CollectDataInRectViaNodeExEvent extends CollectDataInShapeEvent {
	static Logger logger = Logger
			.getLogger(CollectDataInRectViaNodeExEvent.class);
	...
	public CollectDataInRectViaNodeExEvent(NeighborAttachment root,
	...
	public void handle() throws SensornetBaseException {
		if (this.queryNa != null) {
			...
			Rect firstRect = ShapeUtil.firstRectHasNodeInRect(this.getRect(),
					this.traverseRectIterator.getDirection(),
					this.endNeighborAttachment);
			NeighborAttachment firstNode = ShapeUtil.firstNodeInRect(this
					.getRect(), this.traverseRectIterator.getDirection(),
					this.endNeighborAttachment);
			Rect nextRect = this.traverseRectIterator.getNextRect(firstNode,
					firstRect);
			NeighborAttachment nextQueryNa = this.traverseRectIterator
					.getNextNeighborAttachment(firstNode, firstRect);

			if (nextQueryNa != null
					&& this.endNeighborAttachment != nextQueryNa) {
				this.endRect = firstRect.add(nextRect,
						this.traverseRectIterator.getDirection());
				 ...
				 * ().getParam().getANSWER_SIZE(),this.
				 * endNeighborAttachment,this.endNeighborAttachment)); }
				 * this.endNeighborAttachment.setHasSendAnswer(true);
				 * this.getAlg
				 * ().addAnswerAttachment(this.endNeighborAttachment);
				 * 
				 * if(GlobalConstants.getInstance().isDebug()) {
				 * NodeHasQueryResultEvent nodeHasQueryResultEvent = new
				 * NodeHasQueryResultEvent(this.getAlg(),
				 * this.endNeighborAttachment);
