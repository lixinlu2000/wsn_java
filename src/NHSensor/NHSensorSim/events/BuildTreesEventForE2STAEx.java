...
public class BuildTreesEventForE2STAEx extends TreeEvent {
	NodeAttachment[] roots;
	SectorInRect[] sectorInRects;
	NodeAttachment ecstaCoordinator;

	public BuildTreesEventForE2STAEx(NodeAttachment[] roots, NodeAttachment ecstaCoordinator, 
			SectorInRect[] sectorInRects, 
			Rect rect,
		...
		this.rect = rect;
		this.roots = roots;
		this.ecstaCoordinator = ecstaCoordinator;
			...
			for (Iterator it = neighbors.iterator(); it.hasNext();) {
				...
				if (neighbor.getNode().getPos().in(this.getRect())
					...
					if(this.getSectorInRectContainsNode(cur)==this.getSectorInRectContainsNode(neighbor)||
							cur.getRoot()==this.ecstaCoordinator) {
