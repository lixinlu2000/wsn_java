...
import NHSensor.NHSensorSim.shapeTraverse.EDCCircleTraverseRectIteratorCDAFailureAware;

public class EDCCircleTraverseRectEventCDAFailureAwareOld extends
		TraverseRectEvent {
	static Logger logger = Logger
			.getLogger(EDCCircleTraverseRectEventCDAFailureAwareOld.class);
	protected int direction;
	int backupNodeNum = 0;

	public EDCCircleTraverseRectEventCDAFailureAwareOld(Message message,
			Algorithm alg) {
		super(message, alg);
	}

	public EDCCircleTraverseRectEventCDAFailureAwareOld(
	...
	public void handle() throws SensornetBaseException {
		...
		Shape repairShape;
		Shape nextShape;
		EDCCircleTraverseRectIteratorCDAFailureAware rti = new EDCCircleTraverseRectIteratorCDAFailureAware(
		...
		logger.debug("EDCCircleTraverseRectEventCDAFailureAware start");
		do {
			...
			if (nextNa == null) {
				...
				if (curShape.contains(curNa.getPos())) {
					nextNa = rti.getOptimizeNextNeighborAttachment(curNa,
							curShape);
					nextShape = rti.getOptimizedNextShape(curNa, curShape);
				}
			}

			if (nextNa != null) {
				cde = new CollectDataInShapeViaNodeEventFailureAware(curNa,
						nextNa, nextShape, isSendQueryMessage, this.message,
						this.alg);
				this.getAlg().getSimulator().handle(cde);
				this.getRoute().addAll(cde.getRoute());
			}
			...
			if (nextNa != null && !nextNa.isAlive() && curNa != nextNa) {
				repairNa = rti.getRepairedNeighborAttachment(curNa, curShape,
						nextNa, nextShape);
				repairShape = rti.getRepairedShape(curNa, curShape, nextNa,
						nextShape);
		...
		logger.debug("EDCCircleTraverseRectEventCDAFailureAware end");
	}
	...
	public String toString() {
		...
		sb.append("EDCCircleTraverseRectEventCDAFailureAware ");
