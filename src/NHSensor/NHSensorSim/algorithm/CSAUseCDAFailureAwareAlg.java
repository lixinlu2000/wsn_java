...
import NHSensor.NHSensorSim.events.EDCCircleTraverseRectEventCDAFailureAware;
...
public class CSAUseCDAFailureAwareAlg extends IWQEAlg {
	static Logger logger = Logger.getLogger(CSAUseCDAFailureAwareAlg.class);

	public static final String NAME = "CSAUseCDAFailureAware";
	private Vector rects = new Vector();

	public CSAUseCDAFailureAwareAlg(Query query, Network network,
	...
	public CSAUseCDAFailureAwareAlg(Query query, Network network,
		...
		super(query, network, simulator, param, CSAUseCDAFailureAwareAlg.NAME,
				statistics);
	}

	public CSAUseCDAFailureAwareAlg() {
	}
	...
	public boolean gridTraverse() throws SensornetBaseException {
		...
		EDCCircleTraverseRectEventCDAFailureAware edcEvent;
		...
		for (int i = 0; i < this.subQueryRegionNum; i++) {
			...
			edcEvent = new EDCCircleTraverseRectEventCDAFailureAware(cur,
