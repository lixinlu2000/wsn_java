...
import NHSensor.NHSensorSim.events.BuildTreesEventForE2STAEx;
 ...
 *
 */
public class E2STAExAlg extends GPSRAlg {
	static Logger logger = Logger.getLogger(E2STAExAlg.class);
	public static final String NAME = "E2STAEx";
	...
	public E2STAExAlg(Query query, Network network, Simulator simulator,
	...
	public E2STAExAlg(Query query, Network network, Simulator simulator,
		...
		super(query, network, simulator, param, E2STAExAlg.NAME, statistics);
	}
	...
	public void initSectorInRects(int count) {
		Node orig = this.getQuery().getOrig();
		Rect rect = ((Query) this.getQuery()).getRect();
		double startRadian = orig.getPos().bearing(rect.getLT());
		Sector sector;
		...
		double maxAlpha = Radian.relativeTo(
				orig.getPos().bearing(rect.getRT()), startRadian);		
		double middleSita = maxAlpha*2/3;
		double alpha ;
		...
		if(count==1) {
			...
			alpha = (maxAlpha-middleSita) / (count-1);
		}
		for (int i = 0; i < count; i++) {
			if(i==((count-1)/2)) {
				sector = new Sector(orig.getPos(), startRadian, middleSita);
				startRadian += middleSita;
			}
			else {
				sector = new Sector(orig.getPos(), startRadian, alpha);
				startRadian += alpha;
	...
	public boolean multicastQueryOld() throws SensornetBaseException {
		Node orig = this.getQuery().getOrig();
		GPSRAttachment root = (GPSRAttachment) orig.getAttachment(this
				.getName());
		Message queryMessage = new Message(this.getParam()
		...
		GreedyForwardToANodeInSectorInRectNearPosEvent gftse = new GreedyForwardToANodeInSectorInRectNearPosEvent(root,
				root.getPos(), this.sectorInRects[index],  SectorDirection.FAR, queryMessage, this);
		this.getSimulator().handle(gftse);
		this.getRoute().addAll(gftse.getRoute());
		initialNas[index] = (GPSRAttachment) gftse.getLastNode();
		this.getSimulator().handle(new DrawNodeEvent(this, initialNas[index].getNode(),
		...
		NeighborAttachment minShapeFistTryLastNode = gftse.getFirstTryLastNode();
		NeighborAttachment cur = minShapeFistTryLastNode;
		for (int i = index - 1; i >= 0; i--) {
			dse = new DrawShapeEvent(this, initialShapes[i]);
			this.getSimulator().handle(dse);

			gftse = new GreedyForwardToANodeInSectorInRectNearPosEvent(cur, root.getPos(), this.sectorInRects[i],
	...
	protected void buildTree() throws SensornetBaseException {
		...
		BuildTreesEventForE2STAEx bte = new BuildTreesEventForE2STAEx(
				this.initialNas, this.ecstaCoordinator, this.sectorInRects, rect, queryMessage, this);
