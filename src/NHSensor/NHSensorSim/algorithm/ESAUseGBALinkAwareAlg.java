...
import NHSensor.NHSensorSim.events.EDCLinkAwareGridTraverseRectEventGBA;
...
public class ESAUseGBALinkAwareAlg extends IWQEAlg {
	static Logger logger = Logger.getLogger(ESAUseGBALinkAwareAlg.class);

	public static final String NAME = "ESAUseGBALinkAware";
	private Vector rects = new Vector();

	public ESAUseGBALinkAwareAlg(Query query, Network network,
	...
	public ESAUseGBALinkAwareAlg(Query query, Network network,
		...
		super(query, network, simulator, param, ESAUseGBALinkAwareAlg.NAME,
				statistics);
	}

	public ESAUseGBALinkAwareAlg() {
	...
	public boolean gridTraverse() throws SensornetBaseException {
		...
		EDCLinkAwareGridTraverseRectEventGBA edcEvent;
		DrawShapeEvent dse;
		...
		for (int i = 0; i < this.subQueryRegionNum; i++) {
			...
			edcEvent = new EDCLinkAwareGridTraverseRectEventGBA(cur,
