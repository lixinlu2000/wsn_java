...
public class IKNNAlg extends GPSRAlg {
	static Logger logger = Logger.getLogger(IKNNAlg.class);

	public static final String NAME = "IKNN";
	...
	public IKNNAlg(QueryBase query) {
		super(query);
	...
	public IKNNAlg(QueryBase query, Network network, Simulator simulator,
	...
	public IKNNAlg(QueryBase query, Network network, Simulator simulator,
		...
		super(query, network, simulator, param, IKNNAlg.NAME, statistics);
	}
	...
	public boolean itenararyTraverse() throws SensornetBaseException {
		...
		/*
		 * if(curRing.contains(startNode.getNode().getPos())&&!startNode.isHasSendAnswer
		 * ()) { this.getQueryMessage().addMessageSegment( new
		 * MessageSegment(this
		 * .getParam().getANSWER_SIZE(),startNode,startNode));
		 * startNode.setHasSendAnswer(true);
		 * 
		 * if(GlobalConstants.getInstance().isDebug()) { nodeHasQueryResultEvent
		 * = new NodeHasQueryResultEvent(this, startNode);
