...
import NHSensor.NHSensorSim.events.EDCGridTraverseRectEventGBAFailureAware;
 ...
 */
public class ESAUseGBAFailureAwareForSuccessRateAlg extends IWQEAlg {
	static Logger logger = Logger
			.getLogger(ESAUseGBAFailureAwareForSuccessRateAlg.class);

	public static final String NAME = "ESAUseGBAFailureAwareForSuccessRate";
	private Vector rects = new Vector();

	public ESAUseGBAFailureAwareForSuccessRateAlg(Query query, Network network,
	...
	public ESAUseGBAFailureAwareForSuccessRateAlg(Query query, Network network,
		...
		super(query, network, simulator, param,
				ESAUseGBAFailureAwareForSuccessRateAlg.NAME, statistics);
	}

	public ESAUseGBAFailureAwareForSuccessRateAlg() {
	}
	...
	public boolean gridTraverse() throws SensornetBaseException {
		...
		EDCGridTraverseRectEventGBAFailureAware edcEvent;

		for (int i = 0; i < this.subQueryRegionNum; i++) {
			Rect subQueryRegion = (Rect) this.subQueryRegions.elementAt(i);
			edcEvent = new EDCGridTraverseRectEventGBAFailureAware(cur,
