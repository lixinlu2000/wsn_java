 ...
 */
public class HSAAlg extends GPSRAlg {
	static Logger logger = Logger.getLogger(HSAAlg.class);
	public static final String NAME = "HSA";
	private SectorInRect[] sectorInRects;
	Shape[] initialShapes;
	NeighborAttachment[] initialNas;

	public HSAAlg(Query query, Network network,
	...
	public HSAAlg(Query query, Network network,
		...
		super(query, network, simulator, param, HSAAlg.NAME,
				statistics);
	}
	...
	public boolean rdcTraverse() throws SensornetBaseException {
		...
		GPSRAttachment root = (GPSRAttachment) orig.getAttachment(this
				.getName());
		RDCTraverseSectorInRectEventNear event;

		for (int i = 0; i < this.sectorInRects.length; i++) {
			SectorInRect subQueryRegion = (SectorInRect) this.sectorInRects[i];
			
			if(subQueryRegion.contains(initialNas[i].getPos())) {
				GreedyForwardToPointEvent gftpe = new GreedyForwardToPointEvent(
						initialNas[i], root.getPos(),
						new Message(this.getParam().getANSWER_SIZE(), initialNas[i]), this);
				this.getSimulator().handle(gftpe);
				
				initialNas[i].setHasSendAnswer(true);
				this.addAnswerAttachment(initialNas[i]);
			...
			event = new RDCTraverseSectorInRectEventNear(root, initialNas[i],
					subQueryRegion, queryAndDataMessage, this);
