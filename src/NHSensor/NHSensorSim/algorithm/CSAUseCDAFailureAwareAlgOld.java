...
import NHSensor.NHSensorSim.events.EDCCircleTraverseRectEventCDAFailureAwareOld;
...
public class CSAUseCDAFailureAwareAlgOld extends IWQEAlg {
	static Logger logger = Logger.getLogger(CSAUseCDAFailureAwareAlgOld.class);

	public static final String NAME = "CSAUseCDAFailureAwareOld";
	private Vector rects = new Vector();

	public CSAUseCDAFailureAwareAlgOld(Query query, Network network,
	...
	public CSAUseCDAFailureAwareAlgOld(Query query, Network network,
		...
		super(query, network, simulator, param,
				CSAUseCDAFailureAwareAlgOld.NAME, statistics);
	}

	public CSAUseCDAFailureAwareAlgOld() {
	}
	...
	public boolean gridTraverse() throws SensornetBaseException {
		...
		EDCCircleTraverseRectEventCDAFailureAwareOld edcEvent;
		...
		for (int i = 0; i < this.subQueryRegionNum; i++) {
			...
			edcEvent = new EDCCircleTraverseRectEventCDAFailureAwareOld(cur,
